...
uint16_t shiftInSlow(uint8_t dataPin, uint8_t dataPin2, uint8_t clockPin, uint8_t bitOrder) {
    uint8_t value = 0;
    uint8_t value2 = 0;
    uint8_t i;
...
void HX711_2::begin(byte dout, byte dout2, byte pd_sck, byte gain) {
	PD_SCK = pd_sck;
	DOUT = dout;
	DOUT2 = dout2;
...
void HX711_2::read(long* readValues, unsigned long timeout) {
	...
	uint16_t readBits = shiftInSlow(DOUT, DOUT2, PD_SCK, MSBFIRST);
    data[2] = readBits & 0xFF;
    data[5] = readBits >> 8;
    readBits = shiftInSlow(DOUT, DOUT2, PD_SCK, MSBFIRST);
    ...
    value = ( static_cast<unsigned long>(filler) << 24
            ...
            | static_cast<unsigned long>(data[4]) << 8
